...
# from Main import RegistrationForm, LoginForm, BookForm, UploadBook, Contact, DeleteBook, UpdateAccount
...
from Main.main import get_db
from Main.models import User  # Import User model
from Main import app, db, bcrypt  # Import Flask app instance and SQLAlchemy object
from flask_login import login_user, current_user, logout_user, login_required
from Main.recomm import recom, bookdisp
import shutil
...
def delete(isbn_num, file_name):
    ...
    with open(file_name, 'r', newline='', encoding='utf-8') as readFile:
        ...
        for row in reader:
            # If the row doesn't contain the ISBN number, write it to the temporary file
            if isbn_num not in row:
                writer.writerow(row)
